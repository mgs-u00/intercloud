...
from mn.log import info, debug, warn, setLogLevel
from mn.net import Mininet, VERSION
from mn.util import netParse, ipAdd, quietRun
from mn.util import buildTopo
from mn.util import custom, customClass
from mn.term import makeTerm, cleanUpScreens
from mn.node import Controller, RemoteController, NOX, OVSController
from mn.node import CPULimitedHost, Host, Node
from mn.node import OVSSwitch, UserSwitch
from mn.link import TCLink, Intf, Link
from mn.cli import CLI
from mn.moduledeps import moduleDeps
from mn.topo import SingleSwitchTopo, LinearTopo, SingleSwitchReversedTopo
from mn.topolib import TreeTopo
...
def getGeo(city, country, state=""):
    app = geopy.geocoders.Nominatim(user_agent="my-app")
    if state:
        loc = app.geocode(", ".join([city, state, country])).raw
    else:
        loc = app.geocode(", ".join([city, country])).raw
    return [loc['lat'], loc['lon']]

def getDelayFromCoord(loc1, loc2):
    distance = geodesic(loc1, loc2).km
    # Get delay in milliseconds
    delay = str(distance * 1000 / 299792) + "ms"
    return delay
...
class HostDialog(CustomDialog):
    ...
    def body(self, master):
        ...
        # Geo
        Label(self.propFrame, text="City:").grid(row=7, sticky=E)
        self.city = Entry(self.propFrame)
        self.city.grid(row=7, column=1, sticky='nswe', columnspan=3)
        if 'attributes' in self.prefValues and 'city' in self.prefValues['attributes']:
            self.city.insert(0, self.prefValues['attributes']['city'])
        Label(self.propFrame, text="Country:").grid(row=8, sticky=E)
        self.country = Entry(self.propFrame)
        self.country.grid(row=8, column=1, sticky='nswe', columnspan=3)
        if 'attributes' in self.prefValues and 'country' in self.prefValues['attributes']:
            self.country.insert(0, self.prefValues['attributes']['country'])

        # Field for Switch MAC
        Label(self.propFrame, text="MAC Address:").grid(row=9, sticky=E)
        self.macEntry = Entry(self.propFrame)
        self.macEntry.grid(row=9, column=1)
...
class SwitchDialog(CustomDialog):
    ...
    def body(self, master):
        ...
        Label(self.leftfieldFrame, text="Cloud Datacenter:").grid(row=rowCount, sticky=E)
        self.datacenter = IntVar()
        self.datacenterButton = Checkbutton(self.leftfieldFrame, variable=self.datacenter)
        self.datacenterButton.grid(row=rowCount, column=1, sticky=W)
        if 'attributes' in self.prefValues and ('datacenter' not in self.prefValues['attributes'] or self.prefValues['attributes']['datacenter'] == '0'):
        ...
        # Geo
        Label(self.leftfieldFrame, text="City:").grid(row=rowCount, sticky=E)
        self.city = Entry(self.leftfieldFrame)
        self.city.grid(row=rowCount, column=1)
        if 'attributes' in self.prefValues and 'city' in self.prefValues['attributes']:
            self.city.insert(0, self.prefValues['attributes']['city'])
        rowCount += 1
        Label(self.leftfieldFrame, text="Country:").grid(row=rowCount, sticky=E)
        self.country = Entry(self.leftfieldFrame)
        self.country.grid(row=rowCount, column=1)
        if 'attributes' in self.prefValues and 'country' in self.prefValues['attributes']:
            self.country.insert(0, self.prefValues['attributes']['country'])
        rowCount += 1
        ...
        # Field for Switch MAC
        Label(self.leftfieldFrame, text="MAC Address:").grid(row=rowCount, sticky=E)
        self.macEntry = Entry(self.leftfieldFrame)
        self.macEntry.grid(row=rowCount, column=1)
        if 'attributes' in self.prefValues and 'mac' in self.prefValues['attributes']:
            self.macEntry.insert(0, self.prefValues['attributes']['mac'])
...
class LinkDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        # Add check for using geo to set delay
        Label(master, text="Use Node Geo for Delay:").grid(row=6, sticky=E)
        self.useNodeGeo = IntVar()
        self.useNodeGeoButton = Checkbutton(master, variable=self.useNodeGeo)
        self.useNodeGeoButton.grid(row=6, column=1, sticky=W)
        if 'useNodeGeo' not in self.linkValues or self.linkValues['useNodeGeo'] == '0':
            self.useNodeGeoButton.deselect()
        else:
...
class MiniEdit( Frame ):
    ...
    def saveTopology( self ):
        ...
        if len(fileName ) > 0:
            ...
            for link in self.links.values():
                ...
                print(srcName, dstName, str(linkopts))
    ...
    def hostDetails( self, _ignore=None ):
        ...
        if hostBox.result:
            ...
            # New custom attribs additions
            if 'attributes' not in newHostOpts:
                newHostOpts['attributes'] = {}
            ...
            if len(hostBox.result['country']) > 0:
                newHostOpts['attributes']['country'] = hostBox.result['country']
            
            # Generate coordinates
            if 'country' in newHostOpts['attributes'] and 'city' in newHostOpts['attributes']:
                loc = getGeo(newHostOpts['attributes']['city'], newHostOpts['attributes']['country'])
                newHostOpts['attributes']['lat'] = loc[0]
                newHostOpts['attributes']['lon'] = loc[1]
    ...
    def switchDetails( self, _ignore=None ):
        ...
        if switchBox.result:
            ...
            if len(switchBox.result['dpid']) > 0:
                ...
                newSwitchOpts['dpid'] = self.counter + 100
                self.counter += 1
            ...
            # New custom attribs additions
            if 'attributes' not in newSwitchOpts:
                newSwitchOpts['attributes'] = {}


            # Get or generate MAC address
            if len(switchBox.result['mac']) > 0:
                newSwitchOpts['attributes']['mac'] = switchBox.result['mac']
            else:
            ...
            if len(switchBox.result['country']) > 0:
                newSwitchOpts['attributes']['country'] = switchBox.result['country']
            
            # Generate coordinates
            if 'country' in newSwitchOpts['attributes'] and 'city' in newSwitchOpts['attributes']:
                loc = getGeo(newSwitchOpts['attributes']['city'], newSwitchOpts['attributes']['country'])
                newSwitchOpts['attributes']['lat'] = loc[0]
                newSwitchOpts['attributes']['lon'] = loc[1]
    ...
    def linkDetails( self, _ignore=None ):
        ...
        if linkBox.result is not None:
            ...
            linkDetail['linkOpts'].update(linkopts)

            # Set delay based on geo if checked
            if linkDetail['linkOpts']['useNodeGeo'] == '1':
                srcName, dstName = linkDetail['src'][ 'text' ], linkDetail['dest'][ 'text' ]
                if srcName in self.hostOpts:
                    src = self.hostOpts[srcName]
                elif srcName in self.switchOpts:
                    src = self.switchOpts[srcName]
                if dstName in self.hostOpts:
                    dst = self.hostOpts[dstName]
                elif dstName in self.switchOpts:
                    dst = self.switchOpts[dstName]
                if 'attributes' in src and 'attributes' in dst and 'lat' in src['attributes'] and 'lon' in src['attributes'] and 'lat' in dst['attributes'] and 'lon' in dst['attributes']:
                    delay = getDelayFromCoord(tuple((src['attributes']['lat'], src['attributes']['lon'])), tuple((dst['attributes']['lat'], dst['attributes']['lon'])))
                    print("setting delay to " + delay)
                    linkDetail['linkOpts']['delay'] = delay
    ...
    def buildNodes( self, net):
        ...
        for widget in self.widgetToItem:
            ...
            if 'Switch' in tags:
                ...
                if 'attributes' in opts and 'mac' in opts['attributes']:
                    newSwitch = net.addSwitch( name , cls=switchClass, mac=opts['attributes']['mac'], **switchParms)
                else:
        ...
        for widget in self.widgetToItem:
            ...
            if 'Host' in tags:
                ...
                if 'attributes' in opts:
                    newHost.addAttributes(opts['attributes'])
                print(newHost.attributes)
            ...
            if 'Switch' in tags:
                ...
                # add optional attributes
                if 'attributes' in opts:
                    newNode.addAttributes(opts['attributes'])
    ...
    def start( self ):
        ...
        if self.net is None:
            ...
            # result = self.net.run( self.net.pingAll )
            # print(result)
